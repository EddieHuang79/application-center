...
class Admin_user_logic extends Basetool
{
   ...
   public static function update_format( $data )
   {
         ...
         if ( !empty($data) && is_array($data) ) 
         {
            ...
            if ( !empty($data["password"]) ) 
            {
               ...
               $result["password"] = bcrypt(trim($data["password"]));
   ...
   public static function get_data_logic( $id )
   {
      ...
      if ( !empty($id) && is_int($id) ) 
      {
         ...
         $result = array(
            ...
            "service_id"   => isset( $auth[$user_data->role_id] ) ? $auth[$user_data->role_id] : []
         );
   ...
   public static function get_data( $id = 0, $token )
   {
         ...
         try 
         {
            ...
            $auth = Service_logic::has_auth( 6, $token );
      ...
      try 
      {
         ...
         foreach ($data as $row) 
         {
            ...
            $list_data[] = array(
               ...
               "service_id"   => isset( $auth[$row->role_id] ) ? $auth[$row->role_id] : []
            );
   ...
   public static function create_account()
   {
      ...
      try
      {
         ...
         $auth = Service_logic::has_auth( 5, $_POST["_token"] );
   ...
   protected function is_admin( $user_id )
   {
      ...
      return isset($user_id) && $user_id === 1;
